...
- **Run on the PC (2026-09-22):** Cloud agent localhost is not Mina’s Windows box. On the PC: `scripts/run-local.ps1` or `dotnet run --project src/Hitshcm.Web --launch-profile https` after `dotnet dev-certs https --trust`. Browser opens `https://localhost:3000/Account/Login`.
